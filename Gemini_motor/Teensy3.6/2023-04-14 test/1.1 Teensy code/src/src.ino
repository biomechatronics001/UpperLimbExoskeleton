...
int current_limitation = 2;  //(unit Amp)
...
float ratio_L = 0;
float ratio_R = 0;
float arm_elevation_L = 0;
float arm_elevation_R = 0;
...
void loop()
{
  ...
  for (arm_elevation_R=0; arm_elevation_R<=180; arm_elevation_R=arm_elevation_R+0.5)
  {
    if (arm_elevation_R > 20)
    {
    ...
    Serial.print(arm_elevation_R/10); Serial.print("   ");
    Serial.print(moment_arm_R*100); Serial.print("   ");
    Serial.print(torque_bio_R); Serial.print("   ");
    Serial.print(motor_torque_R); Serial.print("   ");
...
void Compute_Cur_Commands()
{
  ...
  else if (assist_mode == 3)
  {
    ...
    if (arm_elevation_L > 20)
    {
      ratio_L = Compute_ratio(arm_elevation_L);
      torque_bio_L = Compute_Biological_Torque(arm_elevation_L);
      Cur_command_L = +Gain_L*Gain_common*torque_bio_L*radius_motor_pulley/radius_arm/ratio_L/Torque_const;
    }
    else
    {
      Cur_command_L = 0;
    ...
    if (arm_elevation_R > 20)
    {
      ratio_R = Compute_ratio(arm_elevation_R);
      torque_bio_R = Compute_Biological_Torque(arm_elevation_R);
      Cur_command_R = -Gain_R*Gain_common*torque_bio_R*radius_motor_pulley/radius_arm/ratio_L / Torque_const;
    }
    else
    {
      Cur_command_R = 0;
