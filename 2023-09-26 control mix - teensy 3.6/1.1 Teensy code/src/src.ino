...
float angle_threshold = 25;
float vel_threshold = 25;
...
float arm_elevation_L_prev;
float arm_elevation_R_prev;
float arm_elevation_L_AV;
float arm_elevation_R_AV;
float arm_elevation_L_AV1;
float arm_elevation_R_AV1;
float arm_elevation_L_AV2;
float arm_elevation_R_AV2;
float arm_elevation_L_AV3;
float arm_elevation_R_AV3;
float arm_elevation_L_AV4;
float arm_elevation_R_AV4;
...
Motor_Control m1(Motor_ID1, CAN_ID, Gear_ratio); //Create motor object see Motor_Control.h
Motor_Control m2(Motor_ID2, CAN_ID, Gear_ratio); //Create motor object see Motor_Control.h
...
void Compute_Cur_Commands()
{
  ...
  {
    mode = "Gravity Compensation based on Position";
    //arm_abduction_L = -imu.LTx;
    //arm_flexion_L = -imu.LTz;
    //arm_abduction_R = -imu.RTx;
    //arm_flexion_R = imu.RTz;

    arm_elevation_L = cos(imu.LTx*PI/180)*cos(imu.LTy*PI/180); arm_elevation_L = acos(arm_elevation_L)*180/PI;
    arm_elevation_R = cos(imu.RTx*PI/180)*cos(imu.RTy*PI/180); arm_elevation_R = acos(arm_elevation_R)*180/PI;
  ...
  {
    mode = "Gravity Compensation based on Position and Velocity";

    //arm_elevation_L_prev = arm_elevation_L;
    //arm_elevation_R_prev = arm_elevation_R;
  
    arm_elevation_L = cos(imu.LTx*PI/180)*cos(imu.LTy*PI/180); arm_elevation_L = acos(arm_elevation_L)*180/PI;
    arm_elevation_R = cos(imu.RTx*PI/180)*cos(imu.RTy*PI/180); arm_elevation_R = acos(arm_elevation_R)*180/PI;
    
    arm_elevation_R_AV = (cos(imu.RTy*PI/180)*sin(imu.RTx*PI/180)*imu.RTAVx + cos(imu.RTx*PI/180)*sin(imu.RTy*PI/180)*imu.RTAVy) / sqrt(1 - pow(cos(imu.RTx*PI/180),2)*pow(cos(imu.RTy*PI/180),2));
    arm_elevation_L_AV = (cos(imu.LTy*PI/180)*sin(imu.LTx*PI/180)*imu.LTAVx + cos(imu.LTx*PI/180)*sin(imu.LTy*PI/180)*imu.LTAVy) / sqrt(1 - pow(cos(imu.LTx*PI/180),2)*pow(cos(imu.LTy*PI/180),2));
    //arm_elevation_L_AV = (arm_elevation_L - arm_elevation_L_prev)/((current_time - previous_time)/1e6);
    //arm_elevation_R_AV = (arm_elevation_R - arm_elevation_R_prev)/((current_time - previous_time)/1e6);

    if (arm_elevation_L > angle_threshold)
    {
      if (arm_elevation_L_AV < -vel_threshold)
      {Cur_command_L = 0;}
      else
      {
      torque_bio_L = Compute_Biological_Torque(arm_elevation_L);
    ...
    if (arm_elevation_R > angle_threshold)
    {
      if (arm_elevation_R_AV < -vel_threshold)
      {Cur_command_R = 0;}
      else
      {
      torque_bio_R = Compute_Biological_Torque(arm_elevation_R);
  ...
  {
    mode = "Gravity Compensation based on Position and integral of Velocity";

    arm_elevation_L_prev = arm_elevation_L;
    arm_elevation_R_prev = arm_elevation_R;

    arm_elevation_L_AV4 = arm_elevation_L_AV3;
    arm_elevation_L_AV3 = arm_elevation_L_AV2;
    arm_elevation_L_AV2 = arm_elevation_L_AV1;
    arm_elevation_L_AV1 = arm_elevation_L_AV;
    
    arm_elevation_R_AV4 = arm_elevation_R_AV3;
    arm_elevation_R_AV3 = arm_elevation_R_AV2;
    arm_elevation_R_AV2 = arm_elevation_R_AV1;
    arm_elevation_R_AV1 = arm_elevation_R_AV;
    ...
    if (arm_elevation_L > angle_threshold)
    {
      ...
      moment_arm_L = Compute_moment_arm(arm_elevation_L)/1000;
      motor_torque_L = torque_bio_L*radius_motor_pulley/moment_arm_L;
      if (arm_elevation_L_AV + arm_elevation_L_AV1*0.8 + arm_elevation_L_AV2*0.6 + arm_elevation_L_AV3*0.4 + arm_elevation_L_AV4*0.2 < -vel_threshold)
      {
        //Cur_command_L = 0;
        Cur_command_L = 0.5*Gain_L*Gain_common*motor_torque_L/Torque_const;
      }
      else
      {
      Cur_command_L = +Gain_L*Gain_common*motor_torque_L/Torque_const;
    ...
    if (arm_elevation_R > angle_threshold)
    {
      ...
      moment_arm_R = Compute_moment_arm(arm_elevation_R)/1000;
      motor_torque_R = torque_bio_R*radius_motor_pulley/moment_arm_R;
      if (arm_elevation_R_AV + arm_elevation_R_AV1*0.8 + arm_elevation_R_AV2*0.6 + arm_elevation_R_AV3*0.4 + arm_elevation_R_AV4*0.2 < -vel_threshold)
      {
        //Cur_command_R = 0;
        Cur_command_R = -0.5*Gain_R*Gain_common*motor_torque_R/Torque_const;
      }
      else
      {
      Cur_command_R = -Gain_R*Gain_common*motor_torque_R/Torque_const;
  ...
  {
    mode = "Gravity Compensation - LX Position, RX Position and integral of Velocity";
  ...
  {
    mode = "Gravity Compensation - LX Position and integral of Velocity, RX Position";
  ...
  else if (assist_mode == 9) //Serial communication with High level system
  {
    ...
    if (data_rs232_rx[0] == 165)
    {
      ...
      if (data_rs232_rx[1] == 90)
      {
        ...
        if (data_rs232_rx[2] == 20)
        {
          ...
          {
            angle_threshold = ((int16_t)(((uint16_t)data_rs232_rx[4]) | ((uint16_t)data_rs232_rx[5] << 8))) / 10.0;
            Serial.print("Angle threshold: ");
            Serial.println(angle_threshold);
          }
          else if (data_rs232_rx[3] == 9)
          {
            vel_threshold = ((int16_t)(((uint16_t)data_rs232_rx[4]) | ((uint16_t)data_rs232_rx[5] << 8))) / 10.0;
            Serial.print("Velocity threshold: ");
            Serial.println(vel_threshold);
          }
...
//  Serial.print(arm_elevation_R_AV); Serial.print("  ");
//  Serial.print(arm_elevation_R_AV1); Serial.print("  ");
//  Serial.print(arm_elevation_R_AV2); Serial.print("  ");
//  Serial.print(arm_elevation_R_AV + arm_elevation_R_AV1*0.66 + arm_elevation_R_AV2*0.33); Serial.print("  ");
//  Serial.println();
}
