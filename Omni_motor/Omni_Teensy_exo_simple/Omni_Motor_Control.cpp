...
void Motor_Control_Tmotor::omni_encoder_reset() {
  msgW.id = (ID<<5) + 0x007;    
  msgW.len = 8;
  msgW.flags.extended = 0;
  msgW.flags.remote   = 0;
...
void Motor_Control_Tmotor::omni_speed_ctl_mode_start() {
  msgW.id = (ID<<5) + 0x00B;  
  msgW.len = 8;   
  msgW.flags.extended = 0;   
  msgW.flags.remote   = 0;   
...
void Motor_Control_Tmotor::omni_torque_ctl_mode_start() {
  msgW.id = (ID<<5) + 0x00B;  
  msgW.len = 8;   
  msgW.flags.extended = 0;   
  msgW.flags.remote   = 0;   
...
void Motor_Control_Tmotor::omni_speed_cmd() {
  msgW.id = (ID<<5) + 0x00D;    
  msgW.len = 8;   
  msgW.flags.extended = 0;   
  msgW.flags.remote   = 0;   
...
void Motor_Control_Tmotor::omni_torque_cmd(float tau) {
  uint8_t bytes[4]; 
  ...
  msgW.buf[6] = 0x00;   
  msgW.buf[7] = 0x00;   
  // Serial.print("Sending torque cmd: ");
  // Serial.println(tau, 6);

  // Serial.print("CAN ID: 0x");
  // Serial.println(msgW.id, HEX);

  // Serial.print("Bytes: ");
  // for (int i = 0; i < msgW.len; ++i) {
  //     Serial.print("0x");
  //     if (msgW.buf[i] < 0x10) Serial.print("0");
  //     Serial.print(msgW.buf[i], HEX);
  //     Serial.print(" ");
  // }
  // Serial.println();
