...
double elevationZ = 0;
double elevationZ_prev = 0;
double elevationZAV = 0;
double elevationZAV_diff = 0;
...
void CurrentControl()
{
  ...
  if (current_time - previous_time >= Tinterval_microsecond) // check if the time period of control loop is already larger than Sample period of control loop (Tinterval_microsecond)
  {
    ...
    elevationZ_prev = elevationZ;
    elevationZ = cos(imu.RTx*PI/180)*cos(imu.RTy*PI/180); elevationZ = acos(elevationZ)*180/PI;
    elevationZAV = (cos(imu.RTy*PI/180)*sin(imu.RTx*PI/180)*imu.RTAVx + cos(imu.RTx*PI/180)*sin(imu.RTy*PI/180)*imu.RTAVy) / sqrt(1 - pow(cos(imu.RTx*PI/180),2)*pow(cos(imu.RTy*PI/180),2));
    elevationZAV_diff = (elevationZ - elevationZ_prev)/(Tinterval_microsecond/1e6);
    //elevationZAV = sqrt(4);
    //elevationZAV_diff = pow(3,2);

    Serial.print(imu.RTAVx); Serial.print(" ");
    Serial.print(imu.RTAVy); Serial.print(" ");
    Serial.print(imu.RTAVz); Serial.print(" ");
    Serial.print(elevationZAV); Serial.print(" ");
    Serial.print(elevationZAV_diff); Serial.print(" ");
    Serial.println();
