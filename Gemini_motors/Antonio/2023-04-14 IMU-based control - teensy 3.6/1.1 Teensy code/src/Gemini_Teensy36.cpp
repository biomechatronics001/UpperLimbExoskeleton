#include "Gemini_Teensy36.h"

Gemini_Teensy36::Gemini_Teensy36(uint32_t id, int Can_id, double Gear_ratio, uint16_t Maxspeed_position)
{
  ID = id;
  gear_ratio = Gear_ratio;
  maxiSpeed = Maxspeed_position * gear_ratio;
}
void Gemini_Teensy36::init_motor()
{
...
void Gemini_Teensy36::DataExplanation(CAN_message_t msgR2)
{
  int len = msgR2.len;
  if (len == 8)
  {
...
void Gemini_Teensy36::write_PID_RAM()
{
  msgW.id = 0x140 + ID;
  msgW.len = 8;
  msgW.flags.extended = 0;
...
void Gemini_Teensy36::write_PID_ROM()
{
  msgW.id = 0x140 + ID;
  msgW.len = 8;
  msgW.flags.extended = 0;
...
//******4.Read Acceleration******//
void Gemini_Teensy36::read_acceleration()
{
  msgW.id = 0x140 + ID;
  msgW.len = 8;
  msgW.flags.extended = 0;
...
void Gemini_Teensy36::write_acceleration_RAM()
{
  msgW.id = 0x140 + ID;
  msgW.len = 8;
  msgW.flags.extended = 0;
...
void Gemini_Teensy36::read_multi_turns_angle()
{
  msgW.id = 0x140 + ID;
  msgW.len = 8;
  msgW.buf[0] = 0x92;
...
void Gemini_Teensy36::send_current_command(double current)
{
  current = current * 100;
...
void Gemini_Teensy36::send_speed_command(double speedvalue)
{
...
void Gemini_Teensy36::send_position_command(double angle)
{
...
void Gemini_Teensy36::receive_CAN_data()
{
  Can0.read(msgR);
