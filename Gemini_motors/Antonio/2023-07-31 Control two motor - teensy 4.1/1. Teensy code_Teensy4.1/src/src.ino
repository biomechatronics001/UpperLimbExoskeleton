...
void CurrentControl()
{
  ...
  {
    Cur_command_1 = -1;//1.5 * sin(2 * PI * current_time / 1000000) / torque_constant;
    Cur_command_2 = 1;//-1.5 * sin(2 * PI * current_time / 1000000) / torque_constant;
    
    m1.send_current_command(Cur_command_1);
    receive_CAN_data();

    m1.send_current_command(Cur_command_2);
    receive_CAN_data();
...
void VelocityControl()
{
  ...
  if (current_time - previous_time > Tinterval_microsecond) // check if the time period of control loop is already larger than Sample period of control loop (Tinterval_microsecond)
  {
    Vel_command_1 = 30 * sin(2 * PI * current_time / 5000000);
    Vel_command_2 = -50 * sin(2 * PI * current_time / 5000000);
    Serial.print(Vel_command_1);
    Serial.print("  ");
    Serial.print(m1.speed_value);
    Serial.print("  ");
    Serial.print(Vel_command_2);
    Serial.print("  ");
    Serial.println(m2.speed_value);
    m1.send_speed_command(Vel_command_1);
    receive_CAN_data();
    m2.send_speed_command(Vel_command_2);
    receive_CAN_data();
...
void PositionControl()
{
  ...
  if (current_time - previous_time > Tinterval_microsecond) // check if the time period of control loop is already larger than Sample period of control loop (Tinterval_microsecond)
  {
    ...
    Pos_command_2 = -40 * sin(2 * PI * current_time / 20000000);
    Serial.print(Pos_command_1);
    Serial.print("  ");
    Serial.print(m1.encoder / Gear_ratio);
    Serial.print("  ");
    Serial.print(Pos_command_2);
    Serial.print("  ");
    Serial.println(m2.encoder / Gear_ratio);

    m1.send_position_command(Pos_command_1);
    receive_CAN_data();
    m2.send_position_command(Pos_command_2);
    receive_CAN_data();
    previous_time = current_time; //reset previous control loop time
  }
}
